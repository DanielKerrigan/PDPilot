...
def get_single_pdps(
    ...
    """calculate one way PDPs for given list of features"""
    results = []
...
def _calc_single_pdp(
    ...
    for value in _get_feature_values(
        ...
        results.append({"x": value, "avg_pred": avg_pred})
...
def get_double_pdps(
    ...
    """calculate two way PDPs for given list of features"""
    results = []
...
def _calc_double_pdp(
    ...
    x_axis = _get_feature_values(
        x_feature, quantitative_features, resolution, unique_feature_vals
    )
    y_axis = _get_feature_values(
        y_feature, quantitative_features, resolution, unique_feature_vals
    )
    ...
    for c, x_value in enumerate(x_axis):
        ...
        for r, y_value in enumerate(y_axis):
            ...
            results.append(
                {"x": x_value, "y": y_value, "row": r, "col": c, "avg_pred": avg_pred}
            )
    ...
    if x_is_quant and y_is_quant:
        kind = "quantitative-double"
    elif x_is_quant or y_is_quant:
        kind = "mixed-double"
    else:
        kind = "categorical-double"
